...
- [RFC-0125](https://github.com/NixOS/rfcs/pull/125) has been merged; [`boot.bootspec.enable`](options.html#opt-boot.bootspec.enable) is now on by default, this means you will have a bootspec document called `boot.json` generated for each system and specialisation in the top-level, e.g. `/run/current-system`. This is useful to enable advanced boot usecases in NixOS such as SecureBoot or to cleanup our bootloader story.
